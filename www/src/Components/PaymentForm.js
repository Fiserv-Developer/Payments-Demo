...
export default class PaymentForm extends Component {
  ...
  submitPayment = () => {
    ...
    fetch("http://localhost:3124/api/v1/payments", requestOptions)
      ...
      .then((data) => {
        ...
        //3DS 2.1
        this.setState({
          iframeCode: data.authenticationResponse.secure3dMethod.methodForm,
        });
  ...
  patchTransactionAfter3DSDataIsSent = () => {
    ...
    // Step 4 (see API)
    fetch(
      "http://localhost:3124/api/v1/payments/" + this.state.transactionId,
      requestOptions
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  sendInCreqData = (data) => {
    ...
    //The cReq data is sent to modirums test ACS. The user will redirected to the ACS after invoking the special post function.
          ...
          >
            <div className="App" style={{ maxWidth: "800px" }}>
              ...
              >
                ...
                {!this.state.buttonEnabled && (
                  <h4>
                    {Math.floor(this.state.timer / 1000).toFixed(0)} seconds
                    remain
                  </h4>
                )}
                {this.state.iframeCode.length > 1 && (
                  <div
